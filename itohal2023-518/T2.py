...
def check_fin_auction(now_time):
    ...
    for _id, time in fin_item:
        print("STATUS", item_table[_id]["status"])
...
# lines = [list(map(str, input().split())) for i in range(11)]
f = open('in.txt', 'r')

lines = f.readlines()
# req_item = ["request_time", "SELL", "user_id",
#             "item_id", "minimum_price", "BIN_price", "end_time"]
# req = {}
...
for s in lines:
    ...
    if request[1] == "SELL":
        ...
        bid_cancel(request)
check_fin_auction("2023-12-31T23:59")
